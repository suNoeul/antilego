...
// ── 08-b: 네트워크 가로채기 ──────────────────────────────────────────────────
// ESV 함수(`/api/esv`)를 실제로 부르지 않고 우리가 정한 응답으로 채운다.
// `Fetch.enable` + `Fetch.fulfillRequest`. 응답에 CORS 헤더를 붙여야 페이지가 읽을 수 있다.
export async function mockFetch(cdp, match, reply) {
  await cdp.send('Fetch.enable', { patterns: [{ urlPattern: match, requestStage: 'Request' }] });
  cdp.on(async m => {
...
export async function clearMocks(cdp) {
  try { await cdp.send('Fetch.disable'); } catch { /* 안 켜져 있었다 */ }
}
