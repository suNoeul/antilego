...
**판정: 통과.** 헤드리스 Chrome 167 단언 전부 PASS (전체를 연달아 두 번 돌려 같은 결과), `console.error` · 예외 · error 로그 **0건**.
...
- ESV(가짜 응답): 절이 그려지고 지명은 개역한글이 가리키는 절에서만, `alt_en`(`Jebus`)도 잡힌다.
  지도와 `이 장에서 N회` 는 **역본과 상관없이 같다** — 그 장의 지명 목록(`places`)은 개역한글에서
  물려받는다 (영어 낱말이 안 맞았다고 그 장에 없던 일이 되지는 않는다).
...
- 실데이터 세 자리(코디네이터 요청): 여호수아 10장 KJV — 43절 · 지명 19곳 · 밑줄 68개
  (`Jerusalem` `Ai` `Jericho` `Gibeon` …). 사도행전 27장 KJV — 8절의 **`The fair havens`** 가
  여러 낱말 그대로 한 버튼. 마태 17장 BSB — 26절, **21절을 조용히 건너뛴다**(빈 절 0 · 에러 0).
- Spike 07 의 검증(`desktop.mjs` · `mobile.mjs` · `fetchfail.mjs`)도 그대로 PASS — 피커 회귀 없음.

스크린샷: [`shots/`](shots) — `b-desktop-*` · `b-real-*` · `b-mobile-*` · `b-legacy-*`.

### 검증하다 고친 것 둘

**① 절 격자가 개수로만 그려지고 있었다.** BSB 에는 사본 이문 16절이 없다 (마 17:21 …).
`1..n` 으로 칸을 만들면 **21번째 칸이 22절을 가리키고 마지막 절(27)에는 닿을 수 없다.**
그래서 절 격자를 **번호 목록**으로 그리게 고쳤다 — 마태 17장 BSB 는 26칸, `21` 칸이 없고
마지막 칸이 `27` 이다. 지금 읽는 장은 **그 역본대로** 세고(없는 절을 권하지 않는다),
다른 장은 정적 역본에서 센다. 절 수 캐시 열쇠에도 역본을 넣었다 — 역본을 바꿨을 때
옛 숫자가 남지 않게.

**② 폰 360px 에서 역본 메뉴가 화면을 2px 넘었다.** 긴 권 이름(데살로니가전서)이면 버튼이
오른쪽으로 밀린다. 폰에서는 메뉴를 버튼에 매달지 않고 화면 폭(`left/right: 8px`)에 맞춘다.
