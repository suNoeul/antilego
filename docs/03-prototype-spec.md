...
## UI 스펙 (B) — Spike 02 개정 (2026-09-18), 02-b 다듬기 (2026-09-18), 02-c 패널 리사이즈 (2026-09-21), 05-b 피드백 (2026-09-21), 06-b 리뷰 반영 (2026-09-21), 08-b 역본 선택 (2026-09-22)
...
- 상단바(높이 48px, sticky): **현재 위치 버튼 `#loc`**(`사사기 9장` + 아주 작은 쉐브론 — `▸` 를 90° 돌린 것),
  그 옆 **역본 버튼 `#verbtn`**(`개역한글 ▾`, 08-b), `‹ ›` 장 이동, 오른쪽 끝 다크모드 토글 `☾`.
  셀렉트는 없다 (Spike 04 에서 `성경 찾기` 로 바뀌었다).
...
- 글꼴: 본문 `"Noto Serif KR", serif` (Google Fonts 링크; 오프라인이면 시스템 serif), UI는 시스템 sans.
  **영문 역본(`lang: "en"`)일 때만** `.verses.lang-en` 이 붙어 시스템 세리프
  (`ui-serif, Georgia, …`)로 바뀌고 줄 사이가 1.9 → **1.7** 로 좁아진다 (08-b).
  제목·지도·카드·피커의 글자는 **언제나 한국어 그대로**다 — 바뀌는 것은 본문뿐이다.
...
- 2줄: **[성경권] [장] [절]** 3열. 열 머리는 10px 옅은 글자이고, 장·절 머리에는 어느 권의
  ...
  - **절**: 그 장의 절 수만큼. **해시는 그 장(`#Book.N`)으로 바뀌고**, 그 절로 스크롤한 뒤
    ...
    절 목록은 장 JSON 에서 가져온다(읽고 있는 장은 이미 받은 것을 쓴다).
    **개수가 아니라 절 번호를 그대로 그린다** (08-b) — 역본에 따라 빠진 절이 있기 때문이다.
    BSB 는 사본 이문 16절이 없어서(마 17:21 …) 마태 17장이 26칸이고, 21 칸이 아예 없고,
    마지막 칸이 **27**이다. 개수만 세면 21번째 칸이 22절을 가리키고 마지막 절에 닿을 수 없다.
    지금 읽고 있는 장은 **그 역본대로** 세고(없는 절을 권하지 않는다), 다른 장은 정적 역본에서 센다.
...
### 역본 선택 (Spike 08-b, 2026-09-22) — `versions.json`

무엇을 읽을지는 **`versions.json` 하나**가 정한다. 역본은 주소에 넣지 않는다 —
해시 문법(`#Book.ch/<placeId>`)은 그대로고, 고른 역본은 `localStorage['ver']` 에 남는다.

```jsonc
{ "default": "krv", "versions": [
  { "id": "krv", "name": "개역한글", "short": "개역한글", "lang": "ko", "type": "static",
    "attribution": "성경전서 개역한글판 © 대한성서공회" },
  { "id": "kjv", "name": "King James Version", "short": "KJV", "lang": "en", "type": "static",
    "attribution": "King James Version (public domain)" },
...
| 키 | 뜻 |
|---|---|
| `type: "static"` | 장 파일이 그대로 있다 → `<id>/books/{Book}/{ch}.json` (스키마는 위와 똑같다, `mentions` 포함) |
| `type: "online"` | 읽을 때마다 우리 함수 `/api/esv` 로 받아온다. **저장하지 않는다** |
| `lang` | `en` 이면 본문 글꼴이 시스템 세리프로 바뀐다 |
| `short` | 상단바 버튼에 쓰는 짧은 이름. `name` 과 같으면 메뉴에서 한 번만 쓴다 |

공용 파일(`index.json` · `places.json` · `attribution.json` · `geo/*` · 시대 3종)은
**역본과 무관하게 `data/` 바로 밑**에 그대로 있다.

#### 버튼 `#verbtn` + 메뉴 `#vermenu`
- 위치 버튼 바로 옆의 작은 알약(12px, 테두리 1px). 누르면 아래에 작은 메뉴가 열린다
  (`role="menu"`, 항목은 `role="menuitemradio"` + `aria-checked`).
- 항목 한 줄 = **이름 + 짧은 이름**, `type: "online"` 이면 옅은 꼬리표 `온라인`.
- 닫는 길: 다시 누르기 · `Esc` · 바깥 클릭 · 항목 고르기. `↑ ↓` 로 항목을 옮긴다.
  **`Esc` 는 역본 메뉴 → 피커 → 지도 패널 순서로 받는다.**
- 폰(≤599px)에서는 메뉴를 버튼 밑에 매달지 않고 `position: fixed` 로 화면 폭에 맞춘다 —
  긴 권 이름(데살로니가전서)이면 버튼이 밀려 메뉴가 화면 밖으로 나가기 때문이다 (가로 스크롤 0).
- `?ver=kjv` 쿼리는 시대 플래그와 같은 길이다 — **한 번 정하고 그대로 저장된다.**
- **역본이 하나뿐이면 버튼을 통째로 숨긴다.**

#### 역본마다 절이 다를 수 있다
BSB 는 사본 이문 16절이 없다 (마 17:21 · 막 7:16 …). 그래서 **절 번호는 1..n 이 아니다.**
본문은 있는 절만 번호 그대로 그린다 — 빈 절도, 안내 문구도 만들지 않는다. 절 격자도
번호를 그대로 쓴다(위 "성경 찾기"). 지도와 `이 장에서 N회` 는 정적 역본(개역한글)의
`places` 를 물려받아 **역본이 달라도 같다** — 영어 낱말이 안 맞았다고 그 장에 없던 일이 되지 않는다.

#### 물러서기 — `versions.json` 이 없으면
못 받으면 옛 배포본으로 보고 `books/{Book}/{ch}.json` 를 **예전 그대로** 읽는다.
역본 버튼은 숨고, 출처 줄도 예전 그대로다. 이 파일 하나 때문에 읽기가 멈추지 않는다.

#### 출처 줄
푸터에 줄 하나(`#attr-ver`)가 더 붙는다 — `본문: <그 역본의 attribution>`.
**출처 목록에 이미 같은 문장이 있으면 띄우지 않는다** (개역한글이 그렇다).
`attribution_url` 이 있으면 뒤에 링크가 붙고, 온라인 역본이면 `note` 가 ` — ` 뒤에 이어진다.
지도 패널(`#panel-attr`)은 자리가 좁으니 **역본 이름만** 앞에 붙인다.

### ESV 온라인 역본 (Spike 08-b) — 받아서 그리고 끝

ESV 본문은 저장소에 넣지 않는다. 무료 API(비상업, AGENTS.md: 결제·문의 없음)에서
**한 장씩 받아 그 자리에서 그리고 버린다.**

- 요청: `GET https://antilego-api.vercel.app/api/esv?ref=Josh.10` → `{ ok, verses:[{v,text}], notice }`.
  함수 쪽 규칙은 [`api/README.md`](../api/README.md).
- **캐시 규칙 (라이선스: 로컬 500절 상한).** 지금 보고 있는 **한 장만** 메모리에 둔다.
  `localStorage` · IndexedDB 에 **넣지 않는다.** 역본을 바꾸거나 장을 옮기면 그 자리에서 사라진다.
  함수 쪽도 마지막 3장 · 60초(총 500절 이하)만 두고 `Cache-Control: private, no-store` 를 준다.
  고지문(`notice`)은 받은 그대로 푸터 출처 줄에 띄운다.

#### 지명은 어떻게 잡나 — 새 색인을 만들지 않는다
온라인 역본에는 `mentions` 가 없다. 그렇다고 영어 본문에서 지명을 마구 찾지 않는다.

1. **어느 절에 어느 지명이 있는지는 정적 역본(개역한글)이 정한다.** 같은 장의 `krv` 파일을
   받아 절마다 `mentions` 의 `p` 목록을 꺼낸다 — OpenBible 판정을 그대로 물려받는 것이다.
2. 그 절에 있다고 적힌 지명만, 영어 본문에서 `places.json` 의 `en` · `alt_en` 로 찾는다.
   `en` 의 동명이지 번호는 떼고 쓴다 (`Jericho 1` → `Jericho`).
3. **낱말 경계**를 맞춘다 (`(?<![A-Za-z0-9])…(?!…)`, 뒤돌아보기를 못 쓰면 `\b`).
   **대소문자를 가린다** — `On`(애굽의 성) 같은 이름이 소문자 `on` 에 붙으면 안 된다.
4. **긴 이름이 이긴다.** 겹치는 자리는 버린다.

그래서 개역한글이 가리키지 않는 절의 지명은 영어 본문에 그 낱말이 있어도 **버튼이 되지 않는다.**
없는 확신을 만들지 않는다 (AGENTS.md 원칙).

#### 못 받았을 때
본문 자리에 한 줄 + 되돌아가는 버튼 하나(`#ver-fallback`, `개역한글로 보기`)를 놓는다.

| 경우 | 문구 |
|---|---|
| `{ok:false, error:"no_key"}` (503) | `ESV API 키가 아직 설정되지 않았습니다` |
| 그 밖 (502 · 네트워크 · JSON 아님) | `ESV 본문을 불러오지 못했습니다 (온라인 전용)` |

### 라우팅 / 상태
- URL 해시: `#Josh.10` (장), `#Josh.10/a231f80` (선택 지명 — `places.json` 키).
  로드 시 해시가 없으면 **유효한 `last` 를 먼저 쓰고, 없거나 망가졌으면 `#Gen.1`**.
- 마지막 읽은 장·다크모드·**지도 패널 열림 여부**·**역본**은 `localStorage`. 실패해도 동작.
  (`last` `theme` `panel` `panelW` `eraLayer` `fbName` `ver`)
- **역본은 주소에 넣지 않는다** (08-b). 해시 문법은 그대로다. `?ver=kjv` 로 한 번 정할 수 있다.
- 장 이동 시 스크롤 맨 위.
